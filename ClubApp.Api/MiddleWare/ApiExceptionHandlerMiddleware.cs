...
namespace ClubApp.Api.MiddleWare
{
    public class ApiExceptionHandlerMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            catch (ConsoleExceptionBase iex)
            {
                await HandleConsoleExceptionAsync(context, iex);
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleConsoleExceptionAsync(HttpContext context, ConsoleExceptionBase iex)
        {
        ...
        private async Task LogExceptionAsync(HttpContext context, Exception ex, int responseStatusCode, string responsePhrase = null)
        {
            ...
            Log.Error(format.ToString(), ex.GetType().FullName, ex.Message, httpRequestDetails, responseStatusCode, responsePhrase, ex.StackTrace);
        }
    }
}
